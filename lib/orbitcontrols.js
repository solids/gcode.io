...
function OrbitControls(scene, camera) {
  ...
  // TODO: this should not live in here....
  this.helperMode = new HelperMode(scene, camera);
  ...
  this.mousedown = function onMouseDown( event ) {
    ...
    if ( event.button === 0 ) {
      ...
      zoomStart.set( event.offsetX, event.offsetY );
  ...
  this.mousewheel = function onMouseWheel( event ) {
  ...
  this.keydown = function onKeyDown( event ) {
...
OrbitControls.prototype.handle = function(type, event) {
  if (typeof this[type] === 'function') {
    var ret = this[type](event);
    if (!ret) {
      return this.helperMode.handle(type, event);
    }
  }
};
